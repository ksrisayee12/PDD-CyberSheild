...
const severityColor: Record<string, string> = {
  Safe: "text-[#A855F7]",
  Moderate: "text-[#FCAF45]",
  High: "text-[#EC4899]",
  Critical: "text-[#FCAF45]",
};
...
export default function OverviewPage() {
  ...
  const changeBadge = () => {
    ...
    if (p > 0) return (
      <span className="flex items-center gap-1 text-xs text-[#FCAF45] font-medium">
    ...
    return (
      <span className="flex items-center gap-1 text-xs text-[hsl(var(--muted-foreground))] font-medium">
  ...
  return (
    <div className="space-y-8">
      <motion.div initial={{ opacity: 0, x: -15 }} animate={{ opacity: 1, x: 0 }}>
        ...
        <p className="page-subtitle">Real-time cyberbullying threat detection and response</p>
      </motion.div>
      ...
      >
        ...
        <motion.div variants={itemVariants}><StatCard label="Messages" value={overview?.total_messages ?? "-"} icon={MessageSquare} color="text-[#A855F7]" /></motion.div>
        <motion.div variants={itemVariants}><StatCard label="Flagged" value={overview?.flagged_content ?? "-"} icon={ShieldAlert} color="text-[#FCAF45]" /></motion.div>
        <motion.div variants={itemVariants}><StatCard label="Critical" value={overview?.critical_alerts ?? "-"} icon={AlertTriangle} color="text-[#EC4899]" /></motion.div>
        <motion.div variants={itemVariants}><StatCard label="Unread" value={overview?.unread_alerts ?? "-"} icon={Bell} color="text-[#E1306C]" /></motion.div>
      </motion.div>
      ...
      {overview && (
        ...
        >
          <div className="glass-panel p-6 rounded-lg relative overflow-hidden group">
            <div className="absolute top-0 left-0 w-1 h-full brand-gradient" />
          ...
          <div className="glass-panel p-6 rounded-lg relative overflow-hidden">
            ...
            <p className={`text-4xl font-black ${overview.percent_change > 0 ? "text-[#FCAF45]" : overview.percent_change < 0 ? "text-[#A855F7]" : "text-[hsl(var(--muted-foreground))]"}`}>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        {/* Monitor control */}
        <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.3 }} className="glass-panel rounded-lg p-5 sm:p-7">
          ...
          {running ? (
            <div className="space-y-3">
              <div className="flex items-center gap-2">
                ...
                <span className="text-[#FCAF45] text-sm font-medium">Monitoring @{running.username}</span>
              </div>
              <p className="text-xs text-[hsl(var(--muted-foreground))]">Session expires 15 min after start.</p>
              <button onClick={stopMonitor} className="px-4 py-2 rounded-lg bg-[#E1306C]/10 text-[#FCAF45] border border-[#E1306C]/25 text-sm hover:bg-[#E1306C]/20 transition-colors">Stop Monitoring</button>
            </div>
          ) : (
            <form onSubmit={startMonitor} className="space-y-3">
              <input value={monForm.instagram_username} onChange={e => setMonForm({ ...monForm, instagram_username: e.target.value })} placeholder="Instagram username" required className="w-full bg-[#050816] border border-[hsl(var(--border))] text-white rounded-lg px-4 py-2.5 text-sm brand-focus" />
              <input type="password" value={monForm.instagram_password} onChange={e => setMonForm({ ...monForm, instagram_password: e.target.value })} placeholder="Instagram password" required className="w-full bg-[#050816] border border-[hsl(var(--border))] text-white rounded-lg px-4 py-2.5 text-sm brand-focus" />
              <button type="submit" disabled={monLoading} className="w-full py-2.5 brand-button text-sm">{monLoading ? "Starting..." : "Start Monitoring"}</button>
            </form>
          )}
        </motion.div>

        {/* Live events */}
        <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.3 }} className="glass-panel rounded-lg p-5 sm:p-7 relative overflow-hidden">
          <div className="flex items-center gap-3 mb-6 relative z-10">
            <span className="w-2.5 h-2.5 bg-[#EC4899] rounded-full animate-pulse" />
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4 }} className="glass-panel rounded-lg p-5 sm:p-8 border-[#EC4899]/30 relative overflow-hidden">
        <div className="absolute -inset-1 bg-gradient-to-r from-[#833AB4]/10 to-[#E1306C]/10 blur-xl z-0 pointer-events-none" />
        <div className="relative z-10">
          <div className="flex flex-wrap items-center gap-3 mb-2">
            <div className="p-2 bg-[#8B5CF6]/10 rounded-lg border border-[#8B5CF6]/25"><FlaskConical className="w-5 h-5 text-[#EC4899]" /></div>
            <h2 className="text-white font-bold text-xl drop-shadow-sm">Manual Content Analysis</h2>
            <span className="sm:ml-2 text-[10px] font-black uppercase tracking-widest px-2.5 py-1 rounded-full bg-[#8B5CF6]/10 text-[#FCAF45] border border-[#8B5CF6]/20">Mode 3 - Safe</span>
          </div>
        <p className="text-xs text-[hsl(var(--muted-foreground))] mb-5">
          Submit any text directly into the AI pipeline - no Instagram required. Runs the full stack: NLP classification to severity to violation tracking to alerts.
        </p>
        <form onSubmit={handleIngest} className="space-y-3">
          <textarea
            ...
            required
            rows={3}
            className="w-full bg-[#050816] border border-[hsl(var(--border))] text-white rounded-lg px-4 py-2.5 text-sm brand-focus resize-none"
          />
          <div className="flex flex-col sm:flex-row gap-3">
            <input
              ...
              required
              className="flex-1 bg-[#050816] border border-[hsl(var(--border))] text-white rounded-lg px-4 py-2.5 text-sm brand-focus"
            />
            <select
              value={ingestForm.content_type}
              onChange={e => setIngestForm({ ...ingestForm, content_type: e.target.value })}
              className="bg-[#050816] border border-[hsl(var(--border))] text-white rounded-lg px-3 py-2.5 text-sm brand-focus"
            >
            ...
            <button
              ...
              className="px-5 py-2.5 brand-button text-sm flex items-center justify-center gap-2"
            >
        ...
        {/* Result panel */}
        {ingestError && (
          <div className="mt-4 p-3 rounded-lg bg-[#E1306C]/10 border border-[#E1306C]/25 text-[#FCAF45] text-sm">{ingestError}</div>
        )}
        {ingestResult && (
